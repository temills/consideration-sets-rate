...
In this experiment, subjects complete 8 trials. In each trial, they are asked to rate items in a given category based on how well they fit certain descriptions.  
The experiment was run on a variety of categories, and the items presented to subjects for comparison are those items that were commonly generated for that category in a previous study, which can be found in the item-generation repo. The descriptions according to which items are rated are relevant descriptors for items in the given category, as determined by subjects' responses in a previous study found in the item-comparisons repo.
Categories, item lists for each category, and descriptor lists for each category can be seen in the index.html file.  

## Data
Response data for the study for each category is in the response-data folder. Some categories have two data files, because the study was run for these categories multiple times with different descriptors. The descriptor-ratings folder contains a json for each category, with the average rating for each item for each descriptor in the given category.  
analyze.py, factor_analysis.py, and animal_coocurrence_analysis.py contain exploratory analysis scripts. 
